...
export function renderMessageContainer(messageContainer, pIdx, step) {
    const playerInfo = this.globalData.players;
    const options = this.api.options;
    const stepFactor = Math.pow(0.99, step);
    const targetMessageAlpha = (options.showMyMessages && playerInfo[pIdx].isMe) || (options.showOthersMessages && !playerInfo[pIdx].isMe) ? 0.9
        : 0;
